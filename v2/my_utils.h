...
static inline my_edge MUmakeEdge(GPoint p0, GPoint p1) {
    my_edge e; e.set(p0, p1);
    return e;
}

static inline void MUclipPoints(GPoint p0, GPoint p1, int w, int h, std::vector<my_edge>& edges) {

    if (p0.fY > p1.fY) std::swap(p0, p1); // reassign so that p0 is always on top
    GPoint *pTop = &p0;
    GPoint *pBottom = &p1;
    ...
    if (pLeft->fX < 0) {
        if (pRight->fX < 0) {
            ...
            pLeft->set(0, ((0 - b)/m));
            edges.push_back(MUmakeEdge(*pLeft, pProjection));
        }    
    }
    ...
    if (pRight->fX > w) {
        if (pLeft->fX > w) {
            ...
            pRight->set(w, ((w - b)/m));
            edges.push_back(MUmakeEdge(*pRight, pProjection));
        }
    }

    my_edge e = MUmakeEdge(*pLeft, *pRight);
    edges.push_back(e);

}

bool LT (my_edge a, my_edge b) {
